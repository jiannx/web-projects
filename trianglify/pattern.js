...
Pattern.prototype = {
    ...
    _getLevel : function(points){
        ...
        for(var m = 0; m< that.opts.level_num; m++){
            for(var k in points){
                if(points[k][0] < (that.boxSize.width * m * 0.1)
                    || points[k][0] > (that.boxSize.width * (1 - m*0.1))
                    || points[k][1] < (that.boxSize.height * m * 0.1)
                    || points[k][1] > (that.boxSize.height * (1 - m*0.1))){
                    return m
                }
            }
        }
